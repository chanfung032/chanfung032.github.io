...
修改网络包后很多时候需要更新相应的 checksum，一般可以增量修改，以修改 iphdr 中的目的地址为例：
...
.. code-block:: c
    ...
    // 调用 bpf_csum_diff 增量计算更新后的 checksum，网络包中的 checksum 字段保存的反码
    // 所以需要先取反再进入计算
    __u32 csum = bpf_csum_diff(old_daddr, sizeof(__be32), &ipv4->daddr, sizeof(__be32), ~ipv4->check);
    // bpf_csum_diff 计算返回的是一个 u32，但是 checksum 需要 u16，需要将高于 16 位的
    // 进位加到低位上去，然后再取反得到反码保存到网络包中
    ipv4->check = csum_fold_helper(csum);

    // 如果是 tcp 包，还需修改内层包的 checksum，因为 tcphdr 中的 checksum 包含了 ip 头
    // 中的部分字段
    // ...
